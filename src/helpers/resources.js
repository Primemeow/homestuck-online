...
import { ASSET_BASE_URL } from '../constants/url'

// ====================================
// Asset resolution

// TODO: fix this incorrect snake case across the whole file
// btw that's the type of TODO that never ever gets resolved
// date of above optimism: 7/15/2023
var assets_root = ASSET_BASE_URL + '/';

export function linkIsOutlink(url) {
    // Regular expression to check if URL starts with http:// or https://
    const startsWithHttpOrHttps = /^http(s{0,1}):\/\//;

    // Regular expression to check if URL is for localhost
    const isLocalhost = /^http(s{0,1}):\/\/localhost/;

    // Regular expression to check if URL is for mspaintadventures.com or its www or cdn subdomains
    const isMspaintadventures = /^http(s{0,1}):\/\/((www|cdn)\.)?mspaintadventures\.com/;

    // The URL is an outlink if it starts with http:// or https://
    // but is not a localhost or mspaintadventures.com URL.
    return startsWithHttpOrHttps.test(url) && 
        !isLocalhost.test(url) && 
        !isMspaintadventures.test(url);
}
  

export function fileIsAsset(url) {
...
export function toFilePath(url, root_dir) {
...
export function resolveAssetsProtocol(asset_url, loopcheck=[]) {
  // Resolves assets:// urls to real urls on the localhost:{port} server
  // TODO: update some comments. in this case we don't serve anything in localhost, we have an actual server
  // thankfully we have a constant for that
  ...
  methods: {
    filterLinksAndImages(el){
      ...
      for (let i = 0; i < media.length; i++) {
        const pseudMediaSrc = media[i].src;
...
export function getStoryNum(pageNumber) {
...
export function getAllPagesInStory(story_id, incl_secret=false) {
  const page_nums = []
  if (story_id == '1'){
...
export function vizToMspa(vizStory, vizPage) {
  let mspaPage
...
export function mspaToViz(mspaInput, isRyanquest = false) {
...
export function isVizBase(base){
